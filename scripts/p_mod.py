...
# Creating a file NUM_mod_rule which will contain the chosen rules:
...
with open(rules, 'r') as file:
	...
	for line in file:
        	...
        	if source == '0.0.0.0/0' and source == dest:
        		continue # Skipping rules where source IP address is 0.0.0.0/0 and is equal to destination IP address.
        
        	val1,val2 = dest_ports.split(':')
        	if int(val1) != int(val2):
                	if int(val1) != 0 or int(val2) != 65535:
                		continue # Skipping rules that do no meet this criteria.
