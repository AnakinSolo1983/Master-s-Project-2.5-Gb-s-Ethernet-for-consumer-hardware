...
# This Bash script manages creates and deletes modified iptables rules within the network namespace ns1, while also monitoring their usage and storing observed statistics. One of the parameters is a pcap file that will be used to generate 'malicious' traffic for this particular test for iptables rules.
...
RSF=result/ns2/pcap/run${RSF} # the path to where the data will be stored.
...
python3 p_mod.py ${NUM} | while read line; do
...
ip netns exec ns1 sudo iptables -L -v -n >> ${RSF}/ipstat.out 2>1
