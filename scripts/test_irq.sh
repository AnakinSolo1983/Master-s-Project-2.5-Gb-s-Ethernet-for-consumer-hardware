...
# This Bash script manages network interrupts for the two network namespaces, ns1 and ns2, acting as server and client respectively. Before and after testing the network performance, the status on the interrupts are captured and compared, showing how interrupt assignment affects network performance.
...
/bin/bash -x test_prf.sh ${TIME} hello 12 1000 ${RSF}
