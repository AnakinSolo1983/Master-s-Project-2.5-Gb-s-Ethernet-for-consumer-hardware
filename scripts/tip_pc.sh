...
# This Bash script manages creates and deletes iptables rules within the network namespace ns1, while also monitoring their usage and storing observed statistics. At the same time, a pcap file will be used to generate "malicious" traffic along with wrk.
...
RSF=result/nft1/pcap/run${RSF} # the path to where the data will be stored.

PCAP=rules_o${NUM}.pcap
